...
public class LlamaModelToolSupportTest {
	...
	public void testToolCalling() {
		...
		/**
		 * .setChatTemplate("{{- bos_token }}\n" + "{%- if custom_tools is defined %}\n"
		 * + " {%- set tools = custom_tools %}\n" + "{%- endif %}\n" + "{%- if not
		 * tools_in_user_message is defined %}\n" + " {%- set tools_in_user_message =
		 ...
		 * functions. To call a function, please respond with JSON for a function
		 ...
		 * the first user message so we can plug it in here #}\n" + " {%- if messages |
		 ...
		 * the following functions, please respond with a JSON for a function call \"
		 * }}\n" + " {{- \"with its proper arguments that best answers the given
		 * prompt.\\n\\n\" }}\n" + " {{- 'Respond in the format {\"name\": function
		 * name, \"parameters\": dictionary of argument name and its value}.' }}\n" + "
		 * {{- \"Do not use variables.\\n\\n\" }}\n" + " {%- for t in tools %}\n" + "
		 * {{- t | tojson(indent=4) }}\n" + " {{- \"\\n\\n\" }}\n" + " {%- endfor %}\n"
		 * + " {{- first_user_message + \"<|eot_id|>\"}}\n" + "{%- endif %}\n" + "\n" +
		 * "{%- for message in messages %}\n" + " {%- if not (message.role == 'ipython'
		 * or message.role == 'tool' or 'tool_calls' in message) %}\n" + " {{-
		 ...
		 * message.content is mapping or message.content is iterable %}\n" + " {{-
